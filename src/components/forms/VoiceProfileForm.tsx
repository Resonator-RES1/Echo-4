...
import { X, Save, Mic2, Plus, Trash2, Fingerprint, Cpu, Sparkles, Users, Loader2, ChevronRight, ChevronLeft, Brain, Quote, Check as CheckIcon } from 'lucide-react';
import { VoiceProfile, Relationship } from '../../types';
import { voiceProfileSchema, VoiceProfileFormValues } from '../../schemas';
...
export function VoiceProfileForm({ onClose, onSave, initialData, isModal = true, voiceProfiles = [], parentId, onCreateEvolution, onEditEvolution }: VoiceProfileFormProps) {
  ...
  }, [initialData, form, voiceCollections, parentId, parentProfile?.name, parentProfile?.gender, parentProfile?.collectionId, parentProfile?.dna, parentProfile?.dnaDescriptions, parentProfile?.goldStandardSnippets]);
        ...
        <div className="flex-1 min-h-0 overflow-hidden">
          <AnimatePresence mode="wait">
            {activeTab === 'history' ? (
              ...
              <div className="space-y-1">
                <label className="text-[9px] font-label uppercase tracking-widest text-on-surface-variant/60 ml-1">Core Motivation {(parentId || initialData?.parentId) && <span className="text-primary/60 italic">(Leave blank to inherit)</span>}</label>
                <Textarea {...form.register('coreMotivation')} placeholder={parentProfile?.coreMotivation || "Core Motivation: What drives them?"} className="bg-surface-container-highest/30 border-outline-variant/20 rounded-lg p-4 min-h-[120px]" />
              </div>
              <div className="space-y-1">
                <label className="text-[9px] font-label uppercase tracking-widest text-on-surface-variant/60 ml-1">Soul Pattern {(parentId || initialData?.parentId) && <span className="text-primary/60 italic">(Leave blank to inherit)</span>}</label>
                <Textarea {...form.register('soulPattern')} placeholder={parentProfile?.soulPattern || "Soul-Pattern: The essence of their voice..."} className="bg-surface-container-highest/30 border-outline-variant/20 rounded-lg p-4 min-h-[120px]" />
              </div>
            </motion.div>
          ) : step === 2 ? (
            <motion.div 
            ...
            >
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div className="space-y-6">
                  <div className="pb-2 border-b border-outline-variant/10 flex items-center gap-2">
                    ...
                    <h3 className="text-sm font-bold text-on-surface">Cognitive & Linguistic</h3>
                  </div>
              <div className="space-y-1">
                <label className="text-[9px] font-label uppercase tracking-widest text-on-surface-variant/60 ml-1">Speech Patterns {(parentId || initialData?.parentId) && <span className="text-primary/60 italic">(Leave blank to inherit)</span>}</label>
              ...
              <div className="space-y-1">
                <label className="text-[9px] font-label uppercase tracking-widest text-on-surface-variant/60 ml-1">Signature Traits</label>
            ...
            <div className="space-y-6">
              <div className="pb-2 border-b border-outline-variant/10 flex items-center gap-2">
                ...
                <h3 className="text-sm font-bold text-on-surface">Behavioral & Social</h3>
              </div>
              <div className="space-y-1">
              ...
              <div className="p-4 bg-surface-container-highest/30 border border-outline-variant/20 rounded-lg space-y-4">
                <div className="flex items-center justify-between">
                  <h4 className="text-sm font-bold text-on-surface">Architectural Guardianship</h4>
